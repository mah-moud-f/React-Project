...
export default function Login() {
    ...
    const handleValidation = (evt) => {
        ...
        if (evt.target.name == "password") {
            ...
            setErrors({ ...errors, passError: (evt.target.value.length == 0) ? "Password is required" : (evt.target.value.length <=  8) ? "password must be at least 8 characters" : "" })



        } else if (evt.target.name == "email") {
            setUser({ ...user, email: evt.target.value })
            setErrors({ ...errors, emailError: (evt.target.value.length == 0) ? "Email is Required" : (evt.target.value.includes("@") ? "" : "Email must includes @") })
        }
      ...
      <div className=" d-flex justify-content-center " >
          <form autoComplete='off' onSubmit={(e) => { handleSubmit(e) }}  className=" bg-white col-4-lg col-6-md col-7-sm rounded p-3 shadow">
            <h4 className='text-center'>Log in to facebook </h4>
                ...
                <div className="">
                    <button type="submit" className="btn btn-primary col-12 btn-lg">log in</button>
            </div>
